...
export function activate(context: vscode.ExtensionContext) {
    ...
    client.onReady().then(() => {
        client.onNotification('error', (params: any) => {
            console.log(
                'Error handling in extension from client has been reached'
            );
        ...
        client.onNotification('unhighlight', (params: any) => {
            ...
            if (!params.fileName) {
                ...
                const existingHighlighter = highlighters.find(highlighter => {
                    return highlighter.editor.document.fileName.includes(
                        params.fileName
                    );
                });
                ...
                currentDocumentFilename =
                    existingHighlighter.editor.document.fileName;
            }
        ...
        vscode.window.showQuickPick(pickerOptions).then(pickedOption => {
            if (!pickedOption) {
                vscode.window.showErrorMessage(
                    'A valid highlight was not selected.'
                );
                return;
            }
        ...
        CredentialManager.getTwitchCredentials()
            .then((creds: TwitchCredentials) => {
                ...
                const chatParams = {
                    ...
                    announce:
                        configuration.get<boolean>('announceBot') || false,
                    joinMessage: configuration.get<string>('joinMessage') || '',
                    leaveMessage:
                        configuration.get<string>('leaveMessage') || ''
                };
                client.sendRequest('startchat', chatParams).then(
                    result => {
                        console.log(
                            'We have begun connection with the Language Server'
                        );
                        vscode.window.showInformationMessage(
            ...
            .catch(reason => {
                vscode.window.showErrorMessage(
                    'Could not start the chat client'
                );
                console.error(
        ...
        if (editor) {
            ...
            if (
                ...
                existingHighlighter.highlights.filter(
                    h =>
                        h.twitchUser === twitchUser &&
                        h.lineNumber === lineNumberInt
                ).length > 0
            ) {
                return;
            }
            ...
            let decoration = {
                range,
                hoverMessage: `From @${
                    twitchUser === 'self' ? 'You' : twitchUser
                }`
            };
            addHighlight(
                existingHighlighter,
    ...
    function triggerUpdateDecorations() {
        ...
        let existingHighlight = highlighters.find(highlight => {
            return (
                highlight.editor.document.fileName ===
