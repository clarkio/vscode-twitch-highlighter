...
export class HighlightManager {
  private readonly _onHighlightsChanged: EventEmitter<HighlightChangedEvent> =
    new EventEmitter();
  ...
  public getDecorations(fileName: string): DecorationOptions[] {
    const idx = this.highlightCollection.findIndex(
      (hc) => hc.fileName === fileName
    );
    if (idx > -1) {
      return this.highlightCollection[idx].highlights.map<DecorationOptions>(
        (h) => {
          return {
            hoverMessage: `From ${h.userName === 'self' ? 'You' : h.userName} ${
              h.comments !== undefined ? h.comments : ''
            }`,
            range: h.range,
          };
        }
  ...
  public add(
    ...
    return new Promise((resolve) => {
      ...
      const idx = this.highlightCollection.findIndex(
        (h) => h.fileName === document.fileName
      );
      if (idx > -1) {
        if (
          !this.highlightCollection[idx].highlights.some(
            (h) =>
              (h.userName === userName || userName === 'self') &&
  ...
  public remove(
    ...
    return new Promise<void>((resolve) => {
      ...
      const idx = this.highlightCollection.findIndex(
        (h) => h.fileName === documentOrFileName
      );
      if (idx > -1) {
        const hidx = this.highlightCollection[idx].highlights.findIndex(
          (h) =>
            (h.userName === userName || userName === 'self') &&
            h.startLine <= lineNumber &&
            h.endLine >= lineNumber
        );
        if (hidx > -1) {
    ...
    return new Promise<void>((resolve) => {
      if (service) {
        this.highlightCollection.forEach((hc) => {
          const highlightsToRemove = hc.highlights.filter(
            (h) => h.userName.indexOf(`${service}:`) > -1
          );
  ...
  public updateHighlight(
    document: TextDocument,
    valueChanged: TextDocumentContentChangeEvent
  ) {
    const idx = this.highlightCollection.findIndex(
      (hc) => hc.fileName === document.fileName
    );
    let updated = false;
    if (idx > -1) {
      // A carriage return was removed.
      if (
        valueChanged.text.length === 0 &&
        valueChanged.range.end.line === valueChanged.range.start.line + 1
      ) {
        ...
        const highlights = this.highlightCollection[idx].highlights.filter(
          (h) => h.range.contains(valueChanged.range)
        );
          ...
          if (valueChanged.text.length === 0) {
            ...
            h.update(
              new Range(
                ...
                new Position(
                  h.range.end.line,
                  h.range.end.character + valueChanged.text.length
                )
              )
            );
            updated = true;
