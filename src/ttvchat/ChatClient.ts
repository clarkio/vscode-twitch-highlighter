...
export class ChatClient implements Disposable {
  private readonly _onChatClientConnected: EventEmitter<boolean> =
    new EventEmitter();
  private readonly _onChatClientMessageReceived: EventEmitter<ChatClientMessageReceivedEvent> =
    new EventEmitter();

  public readonly onChatClientConnected: Event<boolean> =
    this._onChatClientConnected.event;
  ...
  public initialize(context: ExtensionContext) {
    ...
    this.requiredBadges =
      this.config.get<string[]>(Settings.requiredBadges) || [];
  ...
  private async onMessageHandler(
    ...
    badges.follower =
      (await API.isUserFollowingChannel(userState.id!, channel)) === true
        ? '1'
        : '0';

    if (this.requiredBadges.length > 0 && !badges.broadcaster) {
      // Check to ensure the user has a required badge
      const canContinue = this.requiredBadges.some(
        (badge) => badges[badge] === '1'
      );
  ...
  private async onDidChangeConfigurationHandler(
    event: ConfigurationChangeEvent
  ) {
    if (
      event.affectsConfiguration(Configuration.sectionIdentifier) &&
      this.isConnected
    ) {
      await this.disconnect();
      await this.connect();
